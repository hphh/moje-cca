...
export class VykazPracesComponent implements OnInit {
    ...
    nextPracovniDenMenuItems: MenuItem[];
    ...
    ngOnInit() {
        this.kalendarMenuItems = [
            ...
            {
                label: this.getMenuObdobiText(),
                icon: 'fa-calendar',
                command: ( event ) => this.kalendarObdobidialogVisible = true,
                styleClass: 'mojeCcaMenuPeriod'
            }
        ];

        this.nextPracovniDenMenuItems = [
            { label: 'Nový výkaz', icon: 'fa-asterisk', command: ( event ) => this.newVykazPrace( this.nextPracovniDen ) }
        ];
    ...
    private confirmVykazPraces( den: Den ) {
        if ( !this.solveableDay( den ) ) {
            this.toasterService.pop( 'error', 'Den není připraven k potvrzení' );
            return;
        }
    ...
    solveableDay( den: Den ): boolean {
        return den.imisDen.unsolved && ( Math.round(( den.imisDen.odpracovanoHod - den.imisDen.vykazanoHod ) * 1000 ) / 1000 == 0 );
    }
}
